...
const subtitles = [
  {
    ...
    highlights: {
      en: ["captured our imagination", "iconic wildlife spectacles", "environmental indicators"],
...
const VideoLearningPage = () => {
  ...
  const highlightText = (text: string, highlights: string[], colorIndex: number) => {
    let result = text;
    highlights.forEach((highlight, i) => {
      const color = highlightColors[(colorIndex + i) % highlightColors.length];
      const regex = new RegExp(highlight, 'gi');
      result = result.replace(regex, `<span class="${color} hover:underline">${highlight}</span>`);
    });
    ...
    <div className="w-full">
      ...
      <div className="aspect-w-16 aspect-h-9 mb-10 max-w-4xl mx-auto h-[500px]">
        <iframe
          ...
          src="https://www.youtube.com/embed/PnBMdJ5KeHk?enablejsapi=1"
      ...
      <div className="flex justify-end mb-4 space-x-2">
        ...
        <Button
          ...
          className="flex items-center gap-2"
        >
          {showChinese ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  ...
                  <div className="space-y-4">
                    {showEnglish && (
                      <div className="text-gray-900">
                        {highlightText(subtitle.en, subtitle.highlights.en, index)}
                      </div>
                    )}
                    {showChinese && (
                      <div className="text-gray-600">
                        {highlightText(subtitle.cn, subtitle.highlights.cn, index)}
                      </div>
                    )}
                    {!showEnglish && !showChinese && (
                      <div className="text-gray-400 italic text-center py-2">
                        点击显示按钮查看字幕 / Click show button to view subtitles
                      </div>
                    )}
                  </div>
                </CardContent>
